...
export default ({ include, exclude, inlineStyles, render, compilerOptions = defaultCompilerOptions } = {}) => {
    ...
    return {
        ...
        transform: async function transform(code, tplFilePath) {
            if (filter(tplFilePath)) {
                const codeToCompile = inlineStyles ? await replaceLinkWithStyle(code, tplFilePath) : code;
